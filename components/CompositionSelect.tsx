...
import { SHOT_TYPES, SHOT_TYPES_BY_CATEGORY, CATEGORY_DISPLAY_NAMES, findShotType } from '@/constants/shotTypes'

interface CompositionSelectProps {
  value: string
...
export function CompositionSelect({ value, onChange }: CompositionSelectProps) {
  ...
  const currentShotType = findShotType(value)
  const categories = Object.keys(SHOT_TYPES_BY_CATEGORY)

  // Filter shot types based on search term
  const filteredShotTypes = searchTerm
    ? SHOT_TYPES.filter(shot =>
        shot.label.toLowerCase().includes(searchTerm.toLowerCase()) ||
        shot.description.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : []

  const handleSelect = (shotValue: string) => {
    onChange(shotValue)
    setIsOpen(false)
  ...
  return (
    <div className="relative">
      ...
      >
        <div>
          <div className="font-medium text-white">
            {currentShotType?.label || 'Select a shot type...'}
          </div>
          {currentShotType && (
            <div className="text-xs text-slate-400 mt-1">
              {currentShotType.description}
            </div>
          )}
        </div>
        <svg
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-[110] w-full mt-1 bg-slate-800 border border-slate-600 rounded-xl shadow-2xl max-h-96 overflow-hidden">
          {/* Search Bar */}
              ...
              {searchTerm ? (
                <div className="p-2">
                  <div className="text-xs font-semibold text-slate-300 mb-2 px-2">
                    Search Results ({filteredShotTypes.length})
                  </div>
                  {filteredShotTypes.length > 0 ? (
                    filteredShotTypes.map((shotType) => (
                      <button
                        key={shotType.value}
                        type="button"
                        onClick={() => handleSelect(shotType.value)}
                        className={`w-full text-left p-2 rounded hover:bg-slate-700 transition-colors duration-200 ${
                          value === shotType.value ? 'bg-purple-600/20 border-l-2 border-purple-500' : ''
                        }`}
                      >
                    ...
                    <div className="p-4 text-center text-slate-500 text-sm">
                      No shot types found matching "{searchTerm}"
                    </div>
                  )}
                </div>
              ) : selectedCategory ? (
                <div className="p-2">
                  ...
                  {SHOT_TYPES_BY_CATEGORY[selectedCategory].map((shotType) => (
                    <button
                      key={shotType.value}
                      type="button"
                      onClick={() => handleSelect(shotType.value)}
                ...
                <div className="p-4 text-center text-slate-500 text-sm">
                  Select a category to view shot types or use the search above
                </div>
              )}
            </div>
          </div>
